...
extension String {
    ...
    func loadUrlData(resulthandler:@escaping (UIImage?) -> Void)  {
        DispatchQueue.global().async {
            do {
                let html = try String(contentsOf: URL(string: self)!)
                let document = try SwiftSoup.parse(html)
                
                
                if let image = try document.select("link[rel=\"shortcut icon\"]").first() {
                    let href = try image.attr("href")
